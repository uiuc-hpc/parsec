...
static void build_unmatched_events_in_thread(dbp_thread_t *thr)
{
    ...
    while( NULL != e2 ) {
        ...
        if( KEY_IS_END(key2) && !dbp_events_match(e1, e2) ) {
            base_key2 = BASE_KEY(key2);
            if ((base_key2 < 0) || (base_key2 >= dbp_file_nb_dictionary_entries(thr->file))) {
                fprintf(stderr, "key %d invalid for event %"PRId64
                                " at offset %"PRId64" + %"PRId64"\n",
                        key2, i2->current_event_index,
                        i2->current_buffer_position, i2->current_event_position);
            ...
            assert( base_key2 >= 0 );
            assert( base_key2 < dbp_file_nb_dictionary_entries(thr->file) );
