                                    ...
                                    bool sorted)
{
    ...
    do {
        ...
        while (next != sentinel && !(d < distance || A_HIGHER_PRIORITY_THAN_B(next, ring, offset)))
        {
            prev = next;
        ...
        if (sorted && (next == sentinel || !A_HIGHER_PRIORITY_THAN_B(next, ring->list_prev, offset))) {
            if (prev != NULL) {
                prev->list_next = ring;
            } else {
                list = ring;
...
repeat:
    ...
    do {
        ...
        } else if (parsec_update_counted_pointer(&lifo->lifo_head, old_head, lifo->lifo_ghost)) {
    ...
    list = lifo_merge_ring(list, ring, lifo->lifo_ghost, offset, distance, sorted);
...
repeat:
    ...
    do {
        ...
        if (sorted && distance == 0 &&
            ...
            /* restore the ring */
            ring->list_prev->list_next = ring;
        } else if (parsec_atomic_sc_ptr((long*)&lifo->lifo_head.data.item, lifo->lifo_ghost)) {
            /* detached all elements from the lifo */
            break;
        }
        /* DO some kind of pause to release the bus */
