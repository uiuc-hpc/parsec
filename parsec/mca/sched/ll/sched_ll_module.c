...
static int flow_ll_init(parsec_execution_stream_t* es, struct parsec_barrier_t* barrier)
{
    ...
    if( 0 == es->th_id ) {
        ...
        snprintf(event_name, PARSEC_PAPI_SDE_MAX_COUNTER_NAME_LEN, "SCHEDULER::PENDING_TASKS::QUEUE=%d::SCHED=LL", es->virtual_process->vp_id);
        ...
        parsec_papi_sde_add_counter_to_group(event_name, "SCHEDULER::PENDING_TASKS::SCHED=LL", PAPI_SDE_SUM);
    }
#endif
...
static void sched_ll_remove( parsec_context_t *master )
{
    ...
    for(p = 0; p < master->nb_vp; p++) {
        ...
        for(t = 0; t < vp->nb_cores; t++) {
            ...
            PARSEC_PAPI_SDE_UNREGISTER_COUNTER("SCHEDULER::PENDING_TASKS::QUEUE=%d::SCHED=LL", vp->vp_id);
        }
    }
    PARSEC_PAPI_SDE_UNREGISTER_COUNTER("SCHEDULER::PENDING_TASKS::SCHED=LL");
}
