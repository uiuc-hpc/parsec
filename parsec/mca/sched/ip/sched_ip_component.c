 ...
 * Local function
 */
static int sched_ip_component_query(mca_base_module_t **module, int *priority);
 ...
 */
const parsec_sched_base_component_t parsec_sched_ip_component = {
...

    {
        ...
        NULL, /*< No open: sched_ip is always available, no need to check at runtime */
        NULL, /*< No close: open did not allocate any resource, no need to release them */
        sched_ip_component_query,
        /*< specific query to return the module and add it to the list of available modules */
        sched_ip_component_register, /*< Register at least the SDE event names */
	"", /*< no reserve */
    },
    {
...
{
    return (mca_base_component_t *)&parsec_sched_ip_component;
}

static int sched_ip_component_query(mca_base_module_t **module, int *priority)
{
    /* module type should be: const mca_base_module_t ** */
    void *ptr = (void*)&parsec_sched_ip_module;
    *priority = 0;
...
static int sched_ip_component_register(void)
{
     PARSEC_PAPI_SDE_DESCRIBE_COUNTER("SCHEDULER::PENDING_TASKS::SCHED=IP",
                                  "the number of pending tasks for the IP scheduler");
     PARSEC_PAPI_SDE_DESCRIBE_COUNTER("SCHEDULER::PENDING_TASKS::QUEUE=<VPID>::SCHED=IP",
