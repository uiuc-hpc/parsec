...
static int flow_ip_init(parsec_execution_stream_t* es, struct parsec_barrier_t* barrier)
{
    ...
    if( 0 == es->th_id ) {
        ...
        snprintf(event_name, PARSEC_PAPI_SDE_MAX_COUNTER_NAME_LEN,
                 "SCHEDULER::PENDING_TASKS::QUEUE=%d::SCHED=IP", es->virtual_process->vp_id);
        ...
        parsec_papi_sde_add_counter_to_group(event_name, "SCHEDULER::PENDING_TASKS::SCHED=IP", PAPI_SDE_SUM);
    }
#endif
...
static void sched_ip_remove( parsec_context_t *master )
{
    ...
    for(p = 0; p < master->nb_vp; p++) {
        ...
        PARSEC_PAPI_SDE_UNREGISTER_COUNTER("SCHEDULER::PENDING_TASKS::QUEUE=%d::SCHED=IP", p);
    }
    PARSEC_PAPI_SDE_UNREGISTER_COUNTER("SCHEDULER::PENDING_TASKS::SCHED=IP");
}
