...
static void jdf_generate_code_release_deps(const jdf_t *jdf, const jdf_function_entry_t *f, const char *name)
{
    ...
    if( !(f->flags & JDF_FUNCTION_FLAG_NO_SUCCESSORS) ) {
       coutput("  if( action_mask & (PARSEC_ACTION_RELEASE_LOCAL_DEPS | PARSEC_ACTION_GET_REPO_ENTRY) ) {\n"
                ...
                "#endif\n"
                "#if defined(PARSEC_SIM_TIME) || defined(PARSEC_SIM_COMM)\n"
                "    assert(arg.output_entry->critical_priority == 0);\n"
                "    assert(!arg.output_entry->critical);\n"
                "    arg.output_entry->critical_priority = this_task->critical_priority;\n"
                "    arg.output_entry->critical = this_task->critical;\n"
                "#endif\n"
                "#if defined(PARSEC_SIM_TIME)\n"
