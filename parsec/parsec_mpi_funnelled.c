...
{
    parsec_comm_engine_stat_update_active(stat, &cb->start_time, 1,
                                       ...
                                       void *cb_data)
{
    ...
    if(post_in_static_array) {
        ...
        MPI_Isend(remote_memory_handle->mem, remote_memory_handle->count, remote_memory_handle->datatype,
                  ...
                  request);
#if defined(PARSEC_STATS_COMM)
        update_stat_start(cb, &parsec_comm_engine_send_stat);
#endif /* PARSEC_STATS_COMM */
    } else {
...
#if defined(PARSEC_STATS_COMM)
    update_stat_start(cb, &parsec_comm_engine_recv_stat);
...
#if defined(PARSEC_STATS_COMM)
    if(item->post_isend) {
        update_stat_start(&array_of_callbacks[mpi_funnelled_last_active_req], &parsec_comm_engine_send_stat);
    } else {
        ...
        for( idx = 0; idx < outcount; idx++ ) {
            ...
            if (MPI_FUNNELLED_TYPE_AM != cb->type) {
                if (cb->stat_bytes == (size_t)-1) {
                    ...
                    update_stat_end(cb, &parsec_comm_engine_recv_stat);
                } else {
                    update_stat_end(cb, &parsec_comm_engine_send_stat);
                }
            }
#endif /* PARSEC_STATS_COMM */
