                              ...
                              dt_type_desc_t* pTypeDesc )
{
    ...
    pTypeDesc->length = 2 * pData->desc.used + 1 /* for the fake PARSEC_DATATYPE_END_LOOP at the end */;
    pTypeDesc->desc = pElemDesc = (dt_elem_desc_t*)malloc( sizeof(dt_elem_desc_t) * pTypeDesc->length );
    pTypeDesc->used = 0;

    assert( PARSEC_DATATYPE_END_LOOP == pData->desc.desc[pData->desc.used].elem.common.type );

    while( stack_pos >= 0 ) {
        if( PARSEC_DATATYPE_END_LOOP == pData->desc.desc[pos_desc].elem.common.type ) { /* end of the current loop */
            ddt_endloop_desc_t* end_loop = &(pData->desc.desc[pos_desc].end_loop);
            if( 0 != last.count ) {
                CREATE_ELEM( pElemDesc, last.common.type, PARSEC_DATATYPE_FLAG_BASIC,
        ...
        if( PARSEC_DATATYPE_LOOP == pData->desc.desc[pos_desc].elem.common.type ) {
            ddt_loop_desc_t* loop = (ddt_loop_desc_t*)&(pData->desc.desc[pos_desc]);
            int index = GET_FIRST_NON_LOOP( &(pData->desc.desc[pos_desc]) );

            if( loop->common.flags & PARSEC_DATATYPE_FLAG_CONTIGUOUS ) {
                ...
                for( uint32_t i = index+1; i < loop->items; i++ ) {
                    ...
                    if( (current->common.type == PARSEC_DATATYPE_LOOP) ||
                        compress.common.type != current->common.type ) {
                        compress.common.type = PARSEC_DATATYPE_UINT1;
                ...
                if( compress.extent == (ptrdiff_t)(compress.blocklen * parsec_datatype_basicDatatypes[compress.common.type]->size) ) {
            ...
            if( 0 != last.count ) {  /* Generate the pending element */
                ...
                pElemDesc++; nbElems++;
                last.count       = 0;
                last.common.type = PARSEC_DATATYPE_LOOP;
            }
            ...
            pElemDesc++; nbElems++;
            PUSH_STACK( pStack, stack_pos, nbElems, PARSEC_DATATYPE_LOOP, loop->loops, total_disp );
            pos_desc++;
        ...
        while( pData->desc.desc[pos_desc].elem.common.flags & PARSEC_DATATYPE_FLAG_DATA ) {  /* go over all basic datatype elements */
          ...
          fuse_loops:
            if( 0 == last.count ) {  /* first data of the datatype */
                ...
                if( (ptrdiff_t)last.blocklen * (ptrdiff_t)parsec_datatype_basicDatatypes[last.common.type]->size == last.extent ) {
             ...
             */
            if( (last.blocklen * parsec_datatype_basicDatatypes[last.common.type]->size) ==
                (current->blocklen * parsec_datatype_basicDatatypes[current->common.type]->size) ) {
                ddt_elem_desc_t save = last;  /* safekeep the type and blocklen */
                if( last.common.type != current->common.type ) {
                    last.blocklen    *= parsec_datatype_basicDatatypes[last.common.type]->size;
                    last.common.type  = PARSEC_DATATYPE_UINT1;
                }
             ...
             */
            if( (ptrdiff_t)(last.disp + (last.count - 1) * last.extent + last.blocklen * parsec_datatype_basicDatatypes[last.common.type]->size) ==
                current->disp ) {
                if( last.count != 1 ) {
                    CREATE_ELEM( pElemDesc, last.common.type, PARSEC_DATATYPE_FLAG_BASIC,
                ...
                if( last.common.type == current->common.type ) {
                    ...
                    last.blocklen = ((last.blocklen * parsec_datatype_basicDatatypes[last.common.type]->size) +
                                     (current->blocklen * parsec_datatype_basicDatatypes[current->common.type]->size));
                    last.common.type = PARSEC_DATATYPE_UINT1;
                }
                last.extent += current->extent;
...
int32_t parsec_datatype_commit( parsec_datatype_t * pData )
{
    ...
    if( pData->flags & PARSEC_DATATYPE_FLAG_COMMITTED ) return PARSEC_SUCCESS;
    pData->flags |= PARSEC_DATATYPE_FLAG_COMMITTED;
    ...
    if( 0 != pData->size ) {
        ...
        assert( pElem[index].elem.common.flags & PARSEC_DATATYPE_FLAG_DATA );
     ...
     */
    pLast->common.type     = PARSEC_DATATYPE_END_LOOP;
    ...
    /*if( pData->size == (pData->true_ub - pData->true_lb) ) return PARSEC_SUCCESS; */

    (void)parsec_datatype_optimize_short( pData, 1, &(pData->opt_desc) );
