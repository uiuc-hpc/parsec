...
#define DO_DEBUG(INST)  if( parsec_ddt_position_debug ) { INST }
#else
...
static inline void
position_single_block(parsec_convertor_t* CONVERTOR,
                      unsigned char** mem, ptrdiff_t mem_update,
                      size_t* space, size_t space_update,
                      size_t* cnt, size_t cnt_update)
{
    PARSEC_DATATYPE_SAFEGUARD_POINTER( *mem, mem_update, (CONVERTOR)->pBaseBuf,
                                     (CONVERTOR)->pDesc, (CONVERTOR)->count );
    DO_DEBUG( parsec_output( 0, "position( %p, %lu ) => space %lu [prolog]\n",
 ...
 */
static inline void
position_predefined_data( parsec_convertor_t* CONVERTOR,
                          dt_elem_desc_t* ELEM,
                          size_t* COUNT,
                          ...
                          size_t* SPACE )
{
    ...
    size_t cando_count = (*SPACE) / parsec_datatype_basicDatatypes[_elem->common.type]->size;
    size_t do_now, do_now_bytes = parsec_datatype_basicDatatypes[_elem->common.type]->size;
    ...
    if( 1 == _elem->blocklen ) {
        DO_DEBUG( parsec_output( 0, "position( %p, %" PRIsize_t " ) x (count %" PRIsize_t ", extent %ld) => space %lu [prolog]\n",
    ...
    if( 0 != do_now ) {
        ...
        if( 0 != do_now ) {
            ...
            do_now_bytes = do_now * parsec_datatype_basicDatatypes[_elem->common.type]->size;
            ...
            if( do_now == left_in_block )
                _memory += _elem->extent - (_elem->blocklen * parsec_datatype_basicDatatypes[_elem->common.type]->size);
            cando_count -= do_now;
        }
    }
    ...
    if( 0 != do_now ) {
        do_now_bytes = _elem->blocklen * parsec_datatype_basicDatatypes[_elem->common.type]->size;
#if PARSEC_ENABLE_DEBUG
...
int parsec_convertor_generic_simple_position( parsec_convertor_t* pConvertor,
                                            size_t* position )
{
    ...
    DUMP( "parsec_convertor_generic_simple_position( %p, &%ld )\n", (void*)pConvertor, (long)*position );
    ...
    if( iov_len_local > pConvertor->pDesc->size ) {
        ...
        DO_DEBUG( parsec_output( 0, "position before %lu asked %lu data size %lu"
        ...
        DO_DEBUG( parsec_output( 0, "after bConverted %lu remaining count %lu iov_len_local %lu\n",
    ...
    DO_DEBUG( parsec_output( 0, "position start pos_desc %d count_desc %" PRIsize_t " disp %llx\n"
    ...
    while( 1 ) {
        if( PARSEC_DATATYPE_END_LOOP == pElem->elem.common.type ) { /* end of the the entire datatype */
            DO_DEBUG( parsec_output( 0, "position end_loop count %" PRIsize_t " stack_pos %d pos_desc %d disp %lx space %lu\n",
            ...
            DO_DEBUG( parsec_output( 0, "position new_loop count %" PRIsize_t " stack_pos %d pos_desc %d disp %lx space %lu\n",
                                   pStack->count, pConvertor->stack_pos, pos_desc,
                                   pStack->disp, (unsigned long)iov_len_local ); );
        }
        ...
        update_loop_description:  /* update the current state */
            ...
            DO_DEBUG( parsec_output( 0, "position set loop count %" PRIsize_t " stack_pos %d pos_desc %d disp %lx space %lu\n",
                                   pStack->count, pConvertor->stack_pos, pos_desc,
                                   pStack->disp, (unsigned long)iov_len_local ); );
            continue;
        }
 ...
 complete_loop:
    ...
    if( !(pConvertor->flags & CONVERTOR_COMPLETED) ) {
        ...
        DO_DEBUG( parsec_output( 0, "position save stack stack_pos %d pos_desc %d count_desc %" PRIsize_t " disp %llx\n",
                               pConvertor->stack_pos, pStack->index, pStack->count, (unsigned long long)pStack->disp ); );
        return 0;
    }
    return 1;
