...
#define SET_CONTIGUOUS_FLAG( INT_VALUE )     (INT_VALUE) = (INT_VALUE) | (PARSEC_DATATYPE_FLAG_CONTIGUOUS)
#define SET_NO_GAP_FLAG( INT_VALUE )         (INT_VALUE) = (INT_VALUE) | (PARSEC_DATATYPE_FLAG_NO_GAPS)
#define UNSET_CONTIGUOUS_FLAG( INT_VALUE )   (INT_VALUE) = (INT_VALUE) & (~(PARSEC_DATATYPE_FLAG_CONTIGUOUS | PARSEC_DATATYPE_FLAG_NO_GAPS))
...
#define PARSEC_DATATYPE_COMPUTE_REQUIRED_ENTRIES( _pdtAdd, _count, _extent, _place_needed) \
{ \
    if( (_pdtAdd)->flags & PARSEC_DATATYPE_FLAG_PREDEFINED ) { /* add a basic datatype */ \
        ...
        if( (_count) != 1 ) { \
            if( (_place_needed) < (MAX_DT_COMPONENT_COUNT - 2) ) { \
                 ...
                 */ \
                parsec_output( 0, "Too many elements in the datatype. The limit is %ud\n", \
...
#define PARSEC_DATATYPE_LB_UB_CONT( _count, _disp, _old_lb, _old_ub, _old_extent, _new_lb, _new_ub ) \
{ \
    if( 0 == _count ) { \
                           ...
                           size_t count, ptrdiff_t disp, ptrdiff_t extent )
{
     ...
     */
    if( 0 == count ) return PARSEC_SUCCESS;
    ...
    /* Deal with the special markers (PARSEC_DATATYPE_LB and PARSEC_DATATYPE_UB) */
    if( PARSEC_DATATYPE_LB == pdtAdd->id ) {
        pdtBase->bdt_used |= (((uint32_t)1) << PARSEC_DATATYPE_LB);
        if( pdtBase->flags & PARSEC_DATATYPE_FLAG_USER_LB ) {
            pdtBase->lb = LMIN( pdtBase->lb, disp );
        } else {
            pdtBase->lb = disp;
            pdtBase->flags |= PARSEC_DATATYPE_FLAG_USER_LB;
        }
        if( (pdtBase->ub - pdtBase->lb) != (ptrdiff_t)pdtBase->size ) {
            pdtBase->flags &= ~PARSEC_DATATYPE_FLAG_NO_GAPS;
        }
        return PARSEC_SUCCESS; /* Just ignore the PARSEC_DATATYPE_LOOP and PARSEC_DATATYPE_END_LOOP */
    } else if( PARSEC_DATATYPE_UB == pdtAdd->id ) {
        pdtBase->bdt_used |= (((uint32_t)1) << PARSEC_DATATYPE_UB);
        if( pdtBase->flags & PARSEC_DATATYPE_FLAG_USER_UB ) {
            pdtBase->ub = LMAX( pdtBase->ub, disp );
        } else {
            pdtBase->ub = disp;
            pdtBase->flags |= PARSEC_DATATYPE_FLAG_USER_UB;
        }
        if( (pdtBase->ub - pdtBase->lb) != (ptrdiff_t)pdtBase->size ) {
            pdtBase->flags &= ~PARSEC_DATATYPE_FLAG_NO_GAPS;
        }
    ...
    PARSEC_DATATYPE_COMPUTE_REQUIRED_ENTRIES( pdtAdd, count, extent, place_needed );
     ...
     */
    PARSEC_DATATYPE_LB_UB_CONT( count, disp, pdtAdd->lb, pdtAdd->ub, extent, lb, ub );
    ...
    if( !(pdtBase->flags & PARSEC_DATATYPE_FLAG_OVERLAP) ) {
     ...
     */
    if( (pdtAdd->flags ^ pdtBase->flags) & PARSEC_DATATYPE_FLAG_USER_LB ) {
        if( pdtBase->flags & PARSEC_DATATYPE_FLAG_USER_LB ) {
     ...
     */
    if( (pdtBase->flags ^ pdtAdd->flags) & PARSEC_DATATYPE_FLAG_USER_UB ) {
        if( pdtBase->flags & PARSEC_DATATYPE_FLAG_USER_UB ) {
            ub = pdtBase->ub;
        }
        pdtBase->flags |= PARSEC_DATATYPE_FLAG_USER_UB;
    ...
    pdtBase->flags |= PARSEC_DATATYPE_FLAG_DATA;
     ...
     */
    if( (pdtAdd->flags & (PARSEC_DATATYPE_FLAG_PREDEFINED | PARSEC_DATATYPE_FLAG_DATA)) == (PARSEC_DATATYPE_FLAG_PREDEFINED | PARSEC_DATATYPE_FLAG_DATA) ) {
        if( NULL != pdtBase->ptypes )
            pdtBase->ptypes[pdtAdd->id] += count;

        pLast->elem.common.flags     = pdtAdd->flags & ~(PARSEC_DATATYPE_FLAG_COMMITTED);
        ...
        if( extent != (ptrdiff_t)pdtAdd->size ) {  /* not contiguous: let's fix */
            ...
            if( count > 1 ) {  /* gaps around the predefined datatype */
                pLast->elem.common.flags &= ~(PARSEC_DATATYPE_FLAG_CONTIGUOUS | PARSEC_DATATYPE_FLAG_NO_GAPS);
            }
        }
        pdtBase->desc.used++;
    } else {
        ...
        pdtBase->flags |= (pdtAdd->flags & PARSEC_DATATYPE_FLAG_USER_LB);
        pdtBase->flags |= (pdtAdd->flags & PARSEC_DATATYPE_FLAG_USER_UB);
...
build_loop:
            ...
            if( count != 1 ) {
                ...
                CREATE_LOOP_START( pLast, count, pdtAdd->desc.used + 1, extent,
                                   (pdtAdd->flags & ~(PARSEC_DATATYPE_FLAG_COMMITTED)) );
                pdtBase->loops     += 2;
                pdtBase->desc.used += 2;
                pLast++;
            }
            ...
            for( i = 0; i < pdtAdd->desc.used; i++ ) {
                ...
                if( PARSEC_DATATYPE_FLAG_DATA & pLast->elem.common.flags )
                    pLast->elem.disp += disp;
                else if( PARSEC_DATATYPE_END_LOOP == pLast->elem.common.type ) {
            ...
            if( pLoop != NULL ) {
                ...
                assert( pLoop[index].elem.common.flags & PARSEC_DATATYPE_FLAG_DATA );
    ...
    if( (localFlags & PARSEC_DATATYPE_FLAG_CONTIGUOUS)             /* both type were contiguous */
    ...
    if( pdtBase->flags & PARSEC_DATATYPE_FLAG_NO_GAPS ) {
        assert( pdtBase->flags & PARSEC_DATATYPE_FLAG_CONTIGUOUS );
    }
