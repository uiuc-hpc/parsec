...
int parsec_datatype_register_params(void)
{
    ...
    ret = parsec_mca_param_reg_int_name("datatype", "unpack_debug",
                                        "Whether to output debugging information in the ddt unpack functions (nonzero = enabled)",
                                        false, false, &parsec_ddt_unpack_debug);
    if (0 > ret) {
        return ret;
    }

    ret = parsec_mca_param_reg_int_name("datatype", "pack_debug",
                                        "Whether to output debugging information in the ddt pack functions (nonzero = enabled)",
                                        false, false, &parsec_ddt_pack_debug);
    if (0 > ret) {
        return ret;
    }

    ret = parsec_mca_param_reg_int_name("datatype", "raw_debug",
                                        "Whether to output debugging information in the ddt raw functions (nonzero = enabled)",
                                        false, false, &parsec_ddt_raw_debug);
    if (0 > ret) {
        return ret;
    }

    ret = parsec_mca_param_reg_int_name("datatype", "position_debug",
                                        "Non zero lead to output generated by the datatype position functions",
                                        false, false, &parsec_ddt_position_debug);
    if (0 > ret) {
        return ret;
    }

    ret = parsec_mca_param_reg_int_name("datatype", "copy_debug",
                                        "Whether to output debugging information in the ddt copy functions (nonzero = enabled)",
                                        false, false, &parsec_ddt_copy_debug);
    if (0 > ret) {
        return ret;
    }

    ret = parsec_mca_param_reg_int_name("datatype", "verbose",
                                        "Set level of parsec datatype verbosity",
                                        false, false, &parsec_ddt_verbose);
    if (0 > ret) {
        return ret;
    }
